...
router.post("/", auth, async (req, res) => {
  ...
  try {
    // Generate a unique project_id (e.g., using timestamp and user id)
    const project_id = `PRJ-${Date.now()}-${req.user.id}`;
    const newProject = await Project.create({
      ...
      deadline,
      priority,
      created_by: req.user.id, // The logged-in user is the project manager
      teamMembers: [req.user.id], // The creator is automatically a team member
      created_at: new Date(),
    });
    res.status(201).json(newProject);
  } catch (err) {
    if (err.code === 11000 && err.keyPattern && err.keyPattern.project_id) {
      return res.status(400).json({ msg: "Duplicate project_id. Please try again." });
    }
...
// @route   GET api/projects/:id/team-members
// @desc    Get all team members (names) for a project
// @access  Private
router.get('/:id/team-members', auth, async (req, res) => {
  try {
    ...
    // teamMembers is array of emp_id (String)
    // Find users whose id is in project.teamMembers
    // const members = await User.find({ _id: { $in: project.teamMembers } }, 'fullName email');
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const projects = await Project.find({ created_by: req.user.id }).sort({
      createdAt: -1,
    });
    res.json(projects);
  } catch (err) {
